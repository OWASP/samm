...
import {AuthService} from '../../services/auth.service';
 ...
 import { VERSPopupComponent } from '../../popup/popup.component';
...
export class VERSTableComponent implements OnInit {
 ...
 /* open(action,obj,content: any) {
  ...
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.verify_security_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(VERSPopupComponent, {
      width: '1000px',
      height: '600px',
