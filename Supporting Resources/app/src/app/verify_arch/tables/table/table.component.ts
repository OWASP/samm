...
import {AuthService} from '../../services/auth.service';
 ...
 import { VERIPopupComponent } from '../../popup/popup.component';
...
export class VERITableComponent implements OnInit {
 ...
 /* open(action,obj,content: any) {
  ...
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.verify_arch_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
  ...
  "secpractice":'Architecture Assessment'
    };
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(VERIPopupComponent, {
      width: '1000px',
      height: '600px',
