...
import {AuthService} from '../../services/auth.service';
 ...
 import { DSEPopupComponent } from '../../popup/popup.component';
...
export class DSETableComponent implements OnInit {
 ...
 /* open(action,obj,content: any) {
  ...
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.design_security_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
  ...
  updateRowData(row_obj){
    var check={
      "groupname":'Design',
      "secpractice":'Security Requirements'
    };
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(DSEPopupComponent, {
      width: '1000px',
      height: '600px',
