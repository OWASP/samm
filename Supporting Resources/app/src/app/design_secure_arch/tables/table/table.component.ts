...
import {AuthService} from '../../services/auth.service';
 ...
 import { DSAPopupComponent } from '../../popup/popup.component';
...
export class DSATableComponent implements OnInit {
 ...
 /* open(action,obj,content: any) {
  ...
  // Versatile SAMM loader
  
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.design_secure_arch_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
           ...
           for (const stat of  data.all) {
            ...
            //console.log(x);
            //x=x.join('> ')
         // var x=stat.description.replace("\r\n",", ");
          //console.log(x);
  ...
  updateRowData(row_obj){
    var check={
      "groupname":'Design',
      "secpractice":'Secure Architecture'
    };
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(DSAPopupComponent, {
      width: '1000px',
      height: '600px',
