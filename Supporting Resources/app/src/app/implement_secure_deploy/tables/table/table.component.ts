...
import {AuthService} from '../../services/auth.service';
 ...
 import { ISDPopupComponent } from '../../popup/popup.component';
...
export class ISDTableComponent implements OnInit {
...
c:false;
 ...
 /* open(action,obj,content: any) {
  ...
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.implement_secure_deploy_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(ISDPopupComponent, {
      width: '1000px',
      height: '600px',
