...
import {AuthService} from '../../services/auth.service';
 ...
 import { GSMPopupComponent } from '../../popup/popup.component';
...
export class GSMTableComponent implements OnInit {
 ...
 /* open(action,obj,content: any) {
  ...
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.governance_strategy_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(GSMPopupComponent, {
      width: '1000px',
      height: '600px',
