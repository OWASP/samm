...
import {AuthService} from '../../services/auth.service';
 ...
 import { OETPopupComponent } from '../../popup/popup.component';
...
export class OETTableComponent implements OnInit {
 ...
 /* open(action,obj,content: any) {
  ...
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.operate_environment_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
  ...
  updateRowData(row_obj){
    var check={
      ...
      "secpractice":'Environment Management'
    };
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(OETPopupComponent, {
      width: '1000px',
      height: '600px',
