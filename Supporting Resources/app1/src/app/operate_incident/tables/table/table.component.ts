...
import {AuthService} from '../../services/auth.service';
 ...
 import { OICPopupComponent } from '../../popup/popup.component';
...
export class OICTableComponent implements OnInit {
 ...
 /* open(action,obj,content: any) {
  ...
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.operate_incident_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
  ...
  updateRowData(row_obj){
    var check={
      ...
      "secpractice":'Incident Management'
    };
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(OICPopupComponent, {
      width: '1000px',
      height: '600px',
