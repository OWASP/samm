...
import {AuthService} from '../../services/auth.service';
 ...
 import { OOPEPopupComponent } from '../../popup/popup.component';
...
export class OOPETableComponent implements OnInit {
 ...
 /* open(action,obj,content: any) {
  ...
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.operate_operational_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
  ...
  updateRowData(row_obj){
    var check={
      ...
      "secpractice":'Operational Management'
    };
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(OOPEPopupComponent, {
      width: '1000px',
      height: '600px',
