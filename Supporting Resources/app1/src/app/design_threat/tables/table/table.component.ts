...
import {AuthService} from '../../services/auth.service';
 ...
 import { DETPopupComponent } from '../../popup/popup.component';
...
export class DETTableComponent implements OnInit {
 ...
 /* open(action,obj,content: any) {
  ...
  loadtab() {
    //console.log('Loading');
    // alert('hi');
    this.authService.design_threat_table().subscribe(data => {
         if (data.success) {
          // console.log(data);
           this.x = data.values;
  ...
  openDialog(action,obj) {
    obj.action = action;
    const dialogRef = this.dialog.open(DETPopupComponent, {
      width: '1000px',
      height: '600px',
